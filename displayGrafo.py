...
def GrafoFromFile(enderecoArquivo = "", dirigido = False) -> Grafo:
    ...
    with open(enderecoArquivo, newline='') as csvfile:
        reader = csv.reader(csvfile)  # Usa o csv.reader para ler o arquivo CSV
        
        # Pular as primeiras 5 linhas do arquivo
        for _ in range(5):
            next(reader)

        # A partir da linha 6, processar os dados do grafo
        for row in reader:
            if len(row) > 1:
                # Transforma os valores lidos (strings) em inteiros
                a, b = map(int, row)
                grafo.addAresta(a, b)
